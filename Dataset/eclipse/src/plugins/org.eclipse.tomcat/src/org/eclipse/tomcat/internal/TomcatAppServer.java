...
public class TomcatAppServer implements IWebappServer {
    ...
    public synchronized void start(int port, String hostAddress) throws CoreException {
        ...
        try {
            ...
            throw new CoreException(new Status(IStatus.ERROR,
            		...
            		TomcatResources.TomcatAppServer_start, //$NON-NLS-1$
            		exc));
        }
        if (!running) {
            throw new CoreException(new Status(IStatus.ERROR,
                    TomcatPlugin.PLUGIN_ID, IStatus.OK,
                    TomcatResources.TomcatAppServer_start, null)); //$NON-NLS-1$
        }
    }
    ...
    private int updateSslPort(InetAddress iAddress) throws CoreException {
        ...
        if (this.sslPort == 0) {
            this.sslPort = SocketUtil.findUnusedLocalPort(iAddress); //$NON-NLS-1$
            if (this.sslPort == -1) {
                throw new CoreException(new Status(IStatus.ERROR,
                        TomcatPlugin.PLUGIN_ID, IStatus.OK,
                        TomcatResources.TomcatAppServer_start_CannotObtainPort, //$NON-NLS-1$
    ...
    public synchronized void stop() throws CoreException {
        ...
        try {
            ...
            throw new CoreException(new Status(IStatus.ERROR,
                    TomcatPlugin.PLUGIN_ID, IStatus.OK,
                    TomcatResources.TomcatAppServer_sslConnectorRemove, //$NON-NLS-1$
                    exc));
        }
        ...
        try {
            ...
            throw new CoreException(new Status(IStatus.ERROR,
                    TomcatPlugin.PLUGIN_ID, IStatus.OK,
                    TomcatResources.TomcatAppServer_httpConnectorRemove, //$NON-NLS-1$
                    exc));
        }
        ...
        try {
            ...
            throw new CoreException(new Status(IStatus.ERROR,
                    TomcatPlugin.PLUGIN_ID, IStatus.OK,
                    TomcatResources.TomcatAppServer_engineRemove, //$NON-NLS-1$
                    exc));
        }
        ...
        try {
            ...
            throw new CoreException(new Status(IStatus.ERROR,
                    TomcatPlugin.PLUGIN_ID, IStatus.OK,
                    TomcatResources.TomcatAppServer_embeddedStop, //$NON-NLS-1$
                    e));
        }
        isStarted = false;
    }
